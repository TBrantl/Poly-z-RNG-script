...
-- Camera Tab for LD Player compatibility
local CameraTab = Window:CreateTab("Camera", "skull")
...
CameraTab:CreateToggle({
    Name = "🔓 Force 3rd Person (LD Player)",
    CurrentValue = false,
    Flag = "Force3rdPerson",
    Callback = function(state)
        if state then
            task.spawn(function()
                while state do
                    pcall(function()
                        ...
                        -- Ensure camera is in custom mode
                        local camera = workspace.CurrentCamera
...
CameraTab:CreateButton({
    ...
    Callback = function()
        pcall(function()
            ...
            Rayfield:Notify({
                Title = "✅ Camera Reset",
                Content = "Camera restored to default",
                Duration = 3,
                Image = 4483362458
            })
        end)
