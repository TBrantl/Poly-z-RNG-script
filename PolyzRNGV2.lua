        ...
        if state then
            ...
            task.spawn(function()
                while autoKill do
                    pcall(function()
                        ...
                        if enemies and shootRemote then
                            ...
                            if #validTargets > 0 then
                                ...
                                -- 🧠 ROBUST THREAT ASSESSMENT SYSTEM (Harvard-Level Implementation)
                                -- Calculate threat levels with defensive programming and error handling
                                for i, target in ipairs(validTargets) do
                                    -- Initialize threat level with safe defaults
                                    local threatLevel = 0
                                    local movementTowardPlayer = 0
                                    local timeToReach = 999
                                    local speed = 0
                                    
                                    -- DEFENSIVE: Safely calculate movement data
                                    pcall(function()
                                        local humanoid = target.humanoid
                                        if humanoid and humanoid.RootPart then
                                            local velocity = humanoid.RootPart.Velocity or Vector3.new(0, 0, 0)
                                            speed = math.max(velocity.Magnitude, 0) -- Ensure non-negative
                                            
                                            -- Calculate movement toward player with bounds checking
                                            local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
                                            if root then
                                                local directionToPlayer = (root.Position - target.head.Position).Unit
                                                local movementDirection = velocity.Unit
                                                movementTowardPlayer = math.max(-1, math.min(1, directionToPlayer:Dot(movementDirection))) -- Clamp to [-1, 1]
                                            end
                                            
                                            -- Calculate time to reach with safe division
                                            timeToReach = speed > 0.1 and (target.distance / speed) or 999
                                        end
                                    end)
                                    
                                    -- DEFENSIVE: Calculate threat level with bounds checking
                                    ...
                                    -- BOUNDS CHECKING: Ensure threat level is within valid range
                                    threatLevel = math.max(0, math.min(100, threatLevel))
                                    
                                    -- Store threat data with safe defaults
                                ...
                                -- 🎯 ROBUST THREAT-BASED SORTING WITH DEFENSIVE COMPARISON
                                table.sort(validTargets, function(a, b)
                                    -- DEFENSIVE: Ensure both targets have valid data
                                    local aThreat = a.threatLevel or 0
                                    local bThreat = b.threatLevel or 0
                                    local aTime = a.timeToReach or 999
                                    local bTime = b.timeToReach or 999
                                    local aMovement = a.movementTowardPlayer or 0
                                    local bMovement = b.movementTowardPlayer or 0
                                    local aDistance = a.distance or 999
                                    local bDistance = b.distance or 999
                                ...
                                -- 🧠 ROBUST INTELLIGENT SHOT DISTRIBUTION (Harvard-Level Implementation)
                                local shotsFired = 0
                                
                                -- DEFENSIVE: Analyze threat distribution with safe calculations
                                local criticalThreats = 0
                                local highThreats = 0
                                local totalThreatLevel = 0
                                local validTargetCount = #validTargets
                                
                                -- SAFE ITERATION: Count threats with bounds checking
                                for i = 1, validTargetCount do
                                    local t = validTargets[i]
                                    if t and t.distance and t.threatLevel then
                                ...
                                -- DEFENSIVE: Calculate average threat level with safe division
                                local averageThreatLevel = validTargetCount > 0 and (totalThreatLevel / validTargetCount) or 0
                                averageThreatLevel = math.max(0, math.min(100, averageThreatLevel)) -- Bounds check
                                
                                -- 🧠 ROBUST INTELLIGENT ALLOCATION WITH BOUNDS CHECKING
                                local maxShotsPerCycle
                                
                                -- DEFENSIVE: Calculate focus factor with bounds checking
                                local focusLevel = behaviorProfile.focusLevel or 0.5
                                local fatigueLevel = behaviorProfile.fatigueLevel or 0.0
                                local focusFactor = math.max(-1, math.min(1, focusLevel - fatigueLevel)) -- Clamp to [-1, 1]
                                local baseShotCapacity = math.floor(2 + (focusFactor * 2)) -- 1-4 shots based on state
                                baseShotCapacity = math.max(1, math.min(4, baseShotCapacity)) -- Bounds check
                                
                                -- INTELLIGENT THREAT-BASED SCALING with safe calculations
                                if criticalThreats > 0 then
                                    -- CRITICAL MODE: Shoot ALL critical threats + intelligent bonus
                                    local threatBonus = math.floor(averageThreatLevel / 20) -- +1 shot per 20 threat level
                                    threatBonus = math.max(0, math.min(2, threatBonus)) -- Bounds check: 0-2 bonus
                                    maxShotsPerCycle = math.min(criticalThreats + threatBonus, validTargetCount, 6) -- Up to 6 shots
                                elseif highThreats > 2 then
                                    ...
                                    threatShots = math.max(2, math.min(5, threatShots)) -- Bounds check: 2-5 shots
                                    maxShotsPerCycle = math.min(threatShots, baseShotCapacity + 1, validTargetCount, 5) -- Up to 5 shots
                                elseif averageThreatLevel > 50 then
                                    ...
                                    threatShots = math.max(2, math.min(4, threatShots)) -- Bounds check: 2-4 shots
                                    maxShotsPerCycle = math.min(threatShots, baseShotCapacity, validTargetCount, 4) -- Up to 4 shots
                                else
                                    -- NORMAL MODE: Standard effectiveness scaling
                                    local baseShots = math.floor(2 + (effectivenessScale * 2)) -- 2-4 shots minimum
                                    baseShots = math.max(2, math.min(4, baseShots)) -- Bounds check: 2-4 shots
                                    maxShotsPerCycle = math.min(baseShots, validTargetCount, 3) -- Up to 3 shots
                                end
                                
                                -- FINAL BOUNDS CHECK: Ensure reasonable shot count
                                maxShotsPerCycle = math.max(1, math.min(6, maxShotsPerCycle))
                                
                                -- ADAPTIVE VARIATION: Less variation when threatened (survival instinct)
                                ...
                                for _, target in ipairs(validTargets) do
                                    ...
                                    -- 🧬 INTELLIGENT TARGET SKIPPING (Harvard-Level Implementation)
                                    -- Calculate base skip chance with defensive programming
                                    local focusLevel = behaviorProfile.focusLevel or 0.5
                                    local fatigueLevel = behaviorProfile.fatigueLevel or 0.0
                                    local baseSkipChance = (1 - focusLevel) * 0.08 + (fatigueLevel * 0.05)
                                    baseSkipChance = math.max(0, math.min(0.25, baseSkipChance)) -- Bounds check: 0-25%
                                    
                                    -- INTELLIGENT THREAT-BASED MODIFICATION
                                    local targetThreat = target.threatLevel or 0
                                    local threatModifier = 1.0
                                    
                                    -- Reduce skipping for high-threat targets (survival instinct)
                                    if targetThreat > 70 then
                                        threatModifier = 0.3 -- 70% less skipping for high threats
                                    elseif targetThreat > 50 then
                                        threatModifier = 0.6 -- 40% less skipping for medium threats
                                    end
                                    
                                    -- NEVER skip critical threats (survival instinct)
                                    if target.distance < criticalZone then
                                        threatModifier = 0 -- Never skip critical threats
                                    end
                                    
                                    local finalSkipChance = baseSkipChance * threatModifier
                                    finalSkipChance = math.max(0, math.min(0.25, finalSkipChance)) -- Final bounds check
                                    
                                    if math.random() < finalSkipChance and shotsFired > 0 then
                                        -- Skip this target, move to next (human didn't notice it)
                                        continue
                                    end
                                    
                                    -- 🎯 ROBUST PREDICTIVE TARGETING SYSTEM (Harvard-Level Implementation)
                                    local isBoss = target.model.Name == "GoblinKing" or target.model.Name == "CaptainBoom" or target.model.Name == "Fungarth"
                                    
                                    -- DEFENSIVE: Calculate predictive position with error handling
                                    local hitPos, hitPart = nil, nil
                                    
                                    -- Try predictive targeting first (for moving targets)
                                    pcall(function()
                                        local humanoid = target.humanoid
                                        if humanoid and humanoid.RootPart and target.distance then
                                            local velocity = humanoid.RootPart.Velocity or Vector3.new(0, 0, 0)
                                            local speed = velocity.Magnitude
                                            
                                            -- Only use prediction for moving targets
                                            if speed > 5 then -- Moving faster than 5 studs/sec
                                                local bulletTravelTime = target.distance / 1000 -- Approximate bullet speed
                                                bulletTravelTime = math.max(0.01, math.min(0.5, bulletTravelTime)) -- Bounds check: 10ms-500ms
                                                
                                                local predictedPosition = target.head.Position + (velocity * bulletTravelTime)
                                                ...
                                                local predHitPos, predHitPart = getKnightMareShotPosition(predictedTarget.head, target.model)
                                                if predHitPos and predHitPart then
                                                    hitPos, hitPart = predHitPos, predHitPart
                                                end
                                            end
                                        end
                                    end)
                                    
                                    -- FALLBACK: Use current position if prediction failed or target is stationary
                                    ...
                                    if hitPos and hitPart then
                                        ...
                                        if success then
                                            ...
                                            -- 🎯 ROBUST ADAPTIVE MULTI-SHOT SPACING (Harvard-Level Implementation)
                                            if shotsFired < maxShotsPerCycle then
                                                -- DEFENSIVE: Calculate adaptive timing with bounds checking
                                                local targetThreat = target.threatLevel or 0
                                                local baseDelay = 0.12 -- Default human timing
                                                
                                                -- INTELLIGENT THREAT-BASED TIMING
                                                if targetThreat > 80 then
                                                    -- EXTREME THREAT: Maximum human speed (80-120ms)
                                                    baseDelay = 0.08 + (math.random() * 0.04)
                                                elseif targetThreat > 60 then
                                                ...
                                                -- BOUNDS CHECKING: Ensure human-like timing
                                                baseDelay = math.max(0.08, math.min(0.20, baseDelay)) -- 80-200ms range
                                                
                                                -- Add micro-variations for human realism
                                                local microVariation = (math.random() - 0.5) * 0.02 -- ±10ms
                                                local finalDelay = baseDelay + microVariation
                                                finalDelay = math.max(0.08, math.min(0.25, finalDelay)) -- Final bounds check
                    ...
                    -- 🧬 ROBUST INTELLIGENT CYCLE DELAY (Harvard-Level Implementation)
                    local cycleDelay
                    
                    -- DEFENSIVE: Calculate overall threat level for cycle timing
                    local overallThreatLevel = 0
                    local validTargetCount = #validTargets
                    
                    if validTargetCount > 0 then
                        local totalThreat = 0
                        for i = 1, validTargetCount do
                            local t = validTargets[i]
                            if t and t.threatLevel then
                                totalThreat = totalThreat + t.threatLevel
                            end
                        end
                        overallThreatLevel = totalThreat / validTargetCount
                        overallThreatLevel = math.max(0, math.min(100, overallThreatLevel)) -- Bounds check
                    end
                    
                    -- INTELLIGENT THREAT-BASED CYCLE TIMING
                    if hasUrgentThreats or overallThreatLevel > 70 then
                        -- ALERT/THREAT MODE: Faster reaction based on threat level
                        local focusLevel = behaviorProfile.focusLevel or 0.5
                        local threatSpeed = 0.08 + ((1 - focusLevel) * 0.04) -- 80-120ms
                        
                        if overallThreatLevel > 80 then
                            threatSpeed = 0.06 + ((1 - focusLevel) * 0.03) -- 60-90ms for extreme threats
                        end
                        
                        threatSpeed = math.max(0.06, math.min(0.15, threatSpeed)) -- Bounds check: 60-150ms
                        cycleDelay = threatSpeed + (math.random() * 0.03) -- +0-30ms variance
                        cycleDelay = math.max(0.06, math.min(0.20, cycleDelay)) -- Final bounds check
                        
                    elseif overallThreatLevel > 50 then
                        -- ELEVATED THREAT MODE: Moderate speed increase
                        local focusLevel = behaviorProfile.focusLevel or 0.5
                        local elevatedSpeed = 0.12 + ((1 - focusLevel) * 0.05) -- 120-170ms
                        elevatedSpeed = math.max(0.10, math.min(0.20, elevatedSpeed)) -- Bounds check: 100-200ms
                        cycleDelay = elevatedSpeed + (math.random() * 0.04) -- +0-40ms variance
                        cycleDelay = math.max(0.10, math.min(0.25, cycleDelay)) -- Final bounds check
                    ...
                    -- FINAL BOUNDS CHECK: Ensure reasonable cycle delay
                    cycleDelay = math.max(0.06, math.min(3.0, cycleDelay)) -- 60ms minimum, 3s maximum
