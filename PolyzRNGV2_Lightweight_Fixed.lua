...
if not rayfieldSuccess then
    ...
    if not backupSuccess then
        warn("[Lightweight] Backup Rayfield also failed:", backupError)
        return
    end
end
...
local Window = Rayfield:CreateWindow({
    Name = "🚀 FREEZY HUB V2 LIGHTWEIGHT 🚀 | POLY-Z | 🛡️ KnightMare Sync",
    Icon = 71338090068856,
    LoadingTitle = "🚀 Initializing Lightweight V2 System...",
    LoadingSubtitle = "Lightweight Performance + Anti-Cheat Synchronization",
    Theme = "Ocean",
...
-- 🚀 LIGHTWEIGHT INSTANCE SYSTEM
local strengthMultiplier = 1
local instancesLoaded = 0
local lightweightInstances = {}

-- Function to create lightweight auto-kill instance
local function createLightweightInstance()
    local instanceId = #lightweightInstances + 1
    local instance = {
        id = instanceId,
        active = false,
        connection = nil
    ...
    table.insert(lightweightInstances, instance)
    return instance
end

CombatTab:CreateSection("🚀 Lightweight Instance System")
...
CombatTab:CreateToggle({
    Name = "🚀 Load Lightweight Instances",
    CurrentValue = false,
    Flag = "LoadLightweightInstances",
    Callback = function(state)
        if state then
            Rayfield:Notify({
                Title = "🚀 LOADING LIGHTWEIGHT INSTANCES",
                Content = "Loading " .. strengthMultiplier .. " lightweight instances...",
                Duration = 3,
                Image = 4483362458
            })
            ...
            Rayfield:Notify({
                Title = "🚀 LIGHTWEIGHT INSTANCES LOADED",
                Content = "Loaded " .. instancesLoaded .. " lightweight instances!",
                Duration = 4,
                Image = 4483362458
            })
        else
            -- Stop all lightweight instances
            for _, instance in ipairs(lightweightInstances) do
                instance.active = false
                if instance.connection then
                    task.cancel(instance.connection)
                end
            end
            
            Rayfield:Notify({
                Title = "🚀 LIGHTWEIGHT INSTANCES STOPPED",
                Content = "All lightweight instances stopped!",
                Duration = 3,
                Image = 4483362458
            })
        end
