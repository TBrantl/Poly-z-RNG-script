-- Simplified V2 version - removes complex combat loop that's causing issues
        ...
        if state then
            ...
            task.spawn(function()
                while autoKill do
                    pcall(function()
                        ...
                        if enemies and shootRemote then
                            ...
                            if #validTargets > 0 then
                                ...
                                -- Shoot up to 3 targets per cycle
                                local shotsFired = 0
                                local maxShots = math.min(3, #validTargets)
                    ...
                    -- Cycle delay based on effectiveness
                    local cycleDelay = shootDelay
                    if effectivenessLevel >= 95 then
                        cycleDelay = 0.05 -- Superhuman speed
                    elseif effectivenessLevel >= 80 then
                        cycleDelay = 0.1 -- High performance
                    else
                        cycleDelay = 0.2 -- Normal speed
                    end
...
print("✓ Simplified V2 version loaded successfully!")
